...
class SEDPNRModel:
    ...
    def __init__(self, num_agents, q_exponent, agent_params, network_adj_matrix, h, num_steps):
        ...
        self.sigma_i = np.array([p["sigma_i"] for p in agent_params]).reshape(1, -1)
        self.rho_i = np.array([p["rho_i"] for p in agent_params]).reshape(1, -1)
        self.p_i_D_to_P = np.array([p["p_i_D_to_P"] for p in agent_params]).reshape(1, -1)
        self.gamma_i_P_to_R = np.array([p["gamma_i_P_to_R"] for p in agent_params]).reshape(1, -1)
        self.eta_i = np.array([p["eta_i"] for p in agent_params]).reshape(1, -1)
    ...
    def _get_weights_for_alpha(self, alpha: float) -> AdamsMoultonWeights:
        """ Lazily initializes and caches weight calculators for a given alpha. """
        if alpha not in self.weight_calculators:
            self.weight_calculators[alpha] = AdamsMoultonWeights(alpha, self.num_steps)
        return self.weight_calculators[alpha]

    def _calculate_f(self, states: np.ndarray, dynamic_betas: np.ndarray) -> np.ndarray:
    ...
    def step(self, t_idx: int, current_states: np.ndarray, f_history: np.ndarray,
        ...
        weights = self._get_weights_for_alpha(current_alpha)
        h_alpha = self.h ** current_alpha
        
        # --- PREDICTOR ---
        j_indices = np.arange(t_idx + 1)
        predictor_hist_sum = np.dot(f_history.T, weights.predictor_weights[j_indices])
        predicted_states = current_states + h_alpha * predictor_hist_sum.T
        
        # --- CORRECTOR ---
        f_predicted = self._calculate_f(predicted_states, dynamic_betas)
        j_indices_corr = np.arange(1, t_idx + 2)
        corrector_hist_sum = np.dot(f_history[1:].T, weights.corrector_weights[j_indices_corr[::-1][:-1]])
        ...
        f_next = self._calculate_f(next_states, dynamic_betas)
