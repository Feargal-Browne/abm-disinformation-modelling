...
class PlatformAgent:
    ...
    def _calculate_utility(self, M: float, engagement: float, spread_of_disinfo: float) -> float:
        """ Platform's utility function: U = Engagement - Cost(Moderation) - Cost(Damage) """
        engagement_utility = self.engagement_factor * engagement
        moderation_cost = self.moderation_cost_factor * (M ** 2)
    ...
    def optimize_policy(self, total_agents: int, num_sharing_disinfo: int, avg_engagement: float):
        ...
        for M_test in np.linspace(0.0, 1.0, 11):
            ...
            anticipated_spread = (num_sharing_disinfo / total_agents) * (1 - M_test)
            anticipated_engagement = avg_engagement * (1 - 0.5 * M_test)
            
            utility = self._calculate_utility(M_test, anticipated_engagement, anticipated_spread)
        ...
        print(f"Platform '{self.name}' updated moderation policy to: {self.current_moderation_level:.2f}")
