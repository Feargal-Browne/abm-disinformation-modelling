        ...
        for t_idx in range(num_steps - 1):
            ...
            for platform in self.platforms:
                ...
                for i, agent in enumerate(self.agents):
                    if agent.decide_to_share(platform, mod_level):
                        ...
                        dynamic_betas[0, i] = 1.0 # This would be more nuanced in a full multiplex SEDPNR
                        # Successful share increases payoff
                        agent.payoff += 0.1 

            # --- 2. Meta-Adaptation: Dynamic Alpha ---
            # Memory deepens as more agents become infected (P or N)
            infected_proportion = np.sum(y_history[t_idx, 3:5, :]) / self.num_agents
            current_alpha = np.clip(0.95 - 0.2 * infected_proportion, 0.75, 0.95)

            # --- 3. SEDPNR Propagation Phase ---
            if t_idx == 0:
                 f_history[0,:,:] = sedpnr_model._calculate_f(y_history[0,:,:], dynamic_betas)
            ...
            for agent in self.agents:
                if agent.payoff > 0: # Simple check if they shared
                    agent.update_reputation(self.platforms[0], True) # Assume success for now
            ...
            if t_idx % 100 == 0 and t_idx > 0:
                ...
                strat_counts = pd.Series([a.strategy for a in self.agents]).value_counts()
                print(strat_counts)
